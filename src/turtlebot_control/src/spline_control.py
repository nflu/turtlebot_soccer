...
class Controller:
	...
	def __init__(self, turtlebot_frame, world_frame, sub_topic, pub_topic, turtlebot_color, \
		...
		self.interception_pub = rospy.Publisher(pub_topic, PointStamped, queue_size=queue_size)
	...
	def publish_once_from_queue(self):
		if len(self.messages):
			...
			trans = self.tfBuffer.lookup_transform(self.world_frame, self.turtlebot_frame, rospy.Time())
			...
			predictor_planner = Predictor_Planner(velocity.x, curr_point.x, velocity.y, curr_point.y, \
				world_frame=self.world_frame, interception_pub=self.interception_pub)
...
class Plan:
	...
	def theta(self, t):
		print("Xdot, Ydot", self.x_dot(t), self.y_dot(t))
		return np.arctan2(self.y_dot(t), self.x_dot(t))

	def omega(self, t):
		print("Thetas:", self.theta(t + 0.01), self.theta(t))
	...
	def planning_time(self):
		...
		print("max speed, max omega: ", max_speed, max_omega)
...
class Predictor_Planner:
	def __init__(self, m1, b1, m2, b2, world_frame, interception_pub):
		self.m1 = m1
		self.b1 = b1
		self.m2 = m2
		self.b2 = b2
		self.world_frame = world_frame
		self.interception_pub = interception_pub
	...
	def plan_to_intercept(self, robot_state, time_horizon, points_to_check):
		...
		try: 
			...
			for t in ts:
				...
				final_state = [at_t_x, at_t_y, robot_state[2]]
				initial_speed = 0.2
				final_speed = 0.2

				plan = self.planner(robot_state, final_state, initial_speed, final_speed)
				time_achieved = plan.planning_time()
				print("Robot state, final state: ", robot_state, final_state)
				print("Plan parameters: ", plan.a1, plan.b1, plan.c1, plan.d1, plan.a2, plan.b2, plan.c2, plan.d2)
				print("Time achieved, t:", time_achieved, t)
				print("\n")
				...
				if time_achieved <= t:
					point = PointStamped()
					point.point.x = final_state[0]
					point.point.y = final_state[1]
					point.point.z = 0.0
					point.header.stamp = rospy.Time.now()
					point.header.stamp = self.world_frame
					self.interception_pub.publish(point)
					return plan
...
if __name__ == '__main__':
	...
	controller = Controller(turtlebot_frame=turtlebot_frame, world_frame=world_frame, sub_topic=sub_topic, 
							pub_topic=pub_topic, turtlebot_color=turtlebot_color)
	while True:
